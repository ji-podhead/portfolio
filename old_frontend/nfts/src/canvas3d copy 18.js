  ...
  function AllSvgs(){
   ...
   {/* */}      
        <group position={[0,0,0]}>  
          ...
          scale={[[0, 0, 0], [10, 13, 3], [1, 0.002, -2]]}
           position={[[1, 0, 0], [-4500, 3000, 0-1]]}
           rotation={[[0, 0, 0], [-0.00021, 0.00051, 0], [0, 0, 0]]}
           opacity={[0,1,0]} />
...
}   <PerspectiveCamera fov={50}  near={0.001} far={20 }/>
  return (
    <Canvas flat linear shadows camera={{ fov: 80, position: [0, 0, 2000], near: 0.1, far: 20000 }} >
         <Stats showPanel={0} className="stats" />
     <fog attach="fog" args={['black', 0, 40]} />
     <EffectComposer>
          <DepthOfField focusDistance={-0} ></DepthOfField>
          <Bloom mipmapBlur luminanceThreshold={1} radius={0.7} />
        </EffectComposer>   
        <pointLight position={new THREE.Vector3(-50000,-1200,100)} color={[10,10,10]} intensity={0}/>
           <Stage  intensity={1} preset={"rembrandt"}    environment="city" shadows={"contact"} adjustCamera={false}>
       

           <PresentationControls ref={controllRef1} snap global zoom={1}   rotation={[0, 0, 0]} polar={[-Math.PI / 20, Math.PI / 20]} azimuth={[-Math.PI / 40, Math.PI / 20]}>
