...
vec3 effect(vec2 p, vec2 pp) {
  ...
  vec2 pos = vec2(p.x+time*0.3,p.y)*vec2(4.);

  float n = smoothstep(0.,noise(pos),p.x*2.);
  col += l ;
...
//  col *= mix(vec3(0.5), vec3(1.0),smoothstep(-0.9, 0.9, sin(0.25*TAU*p.y/aa+TAU*vec3(0.0, 1., 2.0)/3.0)));
  return col;
      ...
      float textColor(vec2 from, vec2 to, vec2 p)
      {
        ...
        inkNess += smoothstep(0., 5., 1.- (nearLine  +  STROKEWIDTH)); // glow
        return inkNess;
      }
      //-----------------------------------------------------------------------------------
    ...
    // vec3 gradient1 =mix(gl_FragCoord.x/(c3),gl_FragCoord.y/(c3),1.);
       ...
       vec3 col = c1*vec3(0.001);
        ...
        d += D(r()); d += E(r()); d += M(r()); d += O(r()); _();
          ...
          float dither = fract(dot(vec2((time*0.01)+0.,-101.0),gl_FragCoord.xy)/71.0)-0.5;
          uv   -= vec2( 0.1 );
          uv.x *= ratio;
          uv.y *= 0.75;
          ...
          d = max( 0.0, d + gradient * 1.0 );
          d = step( 0.5, d*d + dither ) * 0.8 + 0.2;
         // fragColor = vec4( vec3(d), 1.0 );

         vec3 gradient1 =mix(coord.x/(c1),coord.y/(c3),1.);  
          col += vec3(d*.5, d, d*.85);
     
        col *= vec3(.4, .4, .3) + 0.5*pow(400.0*xy.x*xy.y*(1.0-xy.x)*(1.0-xy.y), .4 );	
      ...
      col2.y = step( 0.5, col2.y*col2.y + dither2 ) * 0.8 + 0.02;
      col2.x = max( 0.0, col2.x + gradient * 1.0 );
      col2.x = step( 0.5, col2.x*col2.x + dither2 ) * 0.1 + 0.01;
      col2.z = max( 0.0, col2.z + gradient * 1.0 );
      col2.z = step( 0.5, col2.z*col2.z + dither2*0.5 ) * 0.1 + 0.02;
    
      col2 /= vec3(.4, .4, .3) + 0.5*pow(100.0*xy.x*xy.y*(1.0-xy.x)*(1.0-xy.y), .1 );	
    col2=mix(col2,colCubes,sin(time*0.2));
      gl_FragColor = vec4( (col2*(c2*0.01) -col*c3*0.05)+ col, 1.0 );
      }
