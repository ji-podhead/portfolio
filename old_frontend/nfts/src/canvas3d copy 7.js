...
//  col *= mix(vec3(0.5), vec3(1.0),smoothstep(-0.9, 0.9, sin(0.25*TAU*p.y/aa+TAU*vec3(0.0, 1., 2.0)/3.0)));
  return col;
    ...
    // vec3 gradient1 =mix(gl_FragCoord.x/(c3),gl_FragCoord.y/(c3),1.);
      ...
      col2.y = step( 0.5, col2.y*col2.y + dither ) * 0.8 + 0.2;
      col2.x = max( 0.0, col2.x + gradient * 1.0 );
      col2.x = step( 0.5, col2.x*col2.x + dither ) * 0.8 + 0.1;
      col2.z = max( 0.0, col2.z + gradient * 1.0 );
      col2.z = step( 0.5, col2.z*col2.z + dither*0.5 ) * 0.8 + 0.2;
    
      col2 /= vec3(.4, .4, .3) + 0.5*pow(100.0*xy.x*xy.y*(1.0-xy.x)*(1.0-xy.y), .1 );	
    col2=mix(col2,colCubes,sin(time*0.2));
      gl_FragColor = vec4( col2*0.5 + col*0.5, 1.0 );
      }
