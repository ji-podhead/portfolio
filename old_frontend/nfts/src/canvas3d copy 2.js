...
const fragmentShader =
  `varying vec2 vUv;
      ...
      float circle(in vec2 _st, in float _radius, in vec2 _center){
        vec2 dist = _st;
        return 1.-smoothstep(_radius-0.002, _radius+0.002, distance(_st, _center));
        }
 ...
 //float circ2=1.-smoothstep(0.5, 0.8, distance(coord, mouse));
       float circ3 = circle(coord,0.8,vec2(0.5,0.5));
        //float circ2 = circle(coord,0.01,vec2(0.5,0.5));
            ...
            vec2 pos = vec2(coord.x*1.5+time*0.3,coord.y)*vec2(5.);
            float n = noise(pos);
            float main = sMin(n,circ1,1.);
            return (sMin(circ3,main,0.1));
           
  }  
  float map2(vec2 coord, vec2 mouse){
    float circ1=1.-smoothstep(-0.5, 5.2, distance(coord, vec2(0.5+sin(time*0.1),0.5+cos(time*0.1))));
    float circ2=1.-smoothstep(0.1, 1.5, distance(coord, vec2(0.5,0.5)));
   //float circ2=1.-smoothstep(0.5, 0.8, distance(coord, mouse));
         float circ3 = circle(coord,0.8,vec2(0.5,0.5));
  ...
  void main()	{
      ...
      vec3 gradient1 =mix(coord.x*(c2*vec3(0.01)),coord.y*(c3),0.4);

      vec2 pos = vec2(coord.x*1.5+time*0.3,coord.y*1.5+time*0.3);
      // Use the noise function
      float n = noise(pos);
      float n2 = noise(vec2(coord.x/2.,coord.y/2.));
      vec3 col1 = mix(c1,gradient1,n)*vec3(n2*vec3(1.,1.,1.));

      float circle1= circle(coord,0.2,mouseFixed);
  ...
  return (
    <Canvas flat linear shadows >
     ...
     <directionalLight intensity={0}></directionalLight>
        <OrbitControls target0={new THREE.Vector3(1000,1000,0) } minDistance={8} enableDamping={true} enablePan={false} maxDistance={10}  ref={controllRef1} snap global zoom={0.5}    rotation={[0, 0, 1]} minAzimuthAngle={-0.30} maxAzimuthAngle={0.30} minPolarAngle={0.8} maxPolarAngle={2} />
